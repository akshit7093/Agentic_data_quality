...
COMPLETE FIX - Added severity and rule_type to ValidationResult
"""
import re
import logging
...
class ValidationEngine:
        ...
        for i, rule in enumerate(rules, 1):
            try:
                ...
                results.append(ValidationResult(
                    ...
                    severity=rule.severity,  # ✅ FIX: Pass severity
                    rule_type=rule.rule_type,  # ✅ FIX: Pass rule_type
                ))
    ...
    async def _execute_column_rule(self, rule: ValidationRule, df: pd.DataFrame) -> ValidationResult:
        ...
        if rule.name == "customer_id_unique" or "unique" in rule.name.lower():
            ...
            # Check phone length or format based on config
            if 'pattern' in config:
                try:
                    matches = col_data.astype(str).str.match(config['pattern'], na=False)
                    failed_mask |= ~matches
            ...
            # Generic logic for backward compatibility
            # Null check
    ...
    async def _execute_table_rule(self, rule: ValidationRule, df: pd.DataFrame) -> ValidationResult:
        ...
        # Special handling for uniqueness
    ...
    async def _execute_statistical_rule(self, rule: ValidationRule, df: pd.DataFrame) -> ValidationResult:
        ...
        # Special handling for date/age validation
    ...
    def _create_result(
        ...
        return ValidationResult(
            ...
            severity=rule.severity,  # ✅ FIX: Pass severity from rule
            rule_type=rule.rule_type,  # ✅ FIX: Pass rule_type from rule
