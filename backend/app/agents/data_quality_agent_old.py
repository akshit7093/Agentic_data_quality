...
class DataQualityAgent:
    ...
    async def _connect_data_source(self, state: AgentState) -> Dict[str, Any]:
        ...
        #     }
        """Connect + smart sampling / full scan."""
        logger.info("=" * 60)
        logger.info("STEP 1: Connecting to data source...")
        full_scan = state.get('execution_config', {}).get('full_scan', False)
        sample_size = state.get('execution_config', {}).get('sample_size', 1000)
        ...
        try:
            ...
            if full_scan and row_count > get_settings().MAX_FULL_SCAN_ROWS:
                logger.warning(f"Full scan requested but {row_count} rows > limit → falling back to sample")
                full_scan = False
                state['messages'].append({"role": "warning", "content": f"Table too large ({row_count} rows). Using sample of {sample_size} rows."})

            if full_scan:
                data = await connector.read_data(
                    state['data_source_info'].target_path, limit=None)
                logger.info(f"FULL SCAN: Loaded all {len(data)} rows")
                print(f"DEBUG FULL_SCAN: {full_scan}, data length = {len(data)}")
            else:
                data = await connector.sample_data(state['data_source_info'].target_path, sample_size=sample_size)
                print(f"DEBUG SAMPLE_DATA: data length = {len(data)}")

            # Update state
            state['data_source_info'].schema = schema
            state['data_source_info'].sample_data = data   # now can be full data
            state['data_source_info'].row_count = row_count
            state['data_source_info'].full_scan_used = full_scan
            print(f"DEBUG ASSIGNMENT: sample_data length is {len(state['data_source_info'].sample_data) if state['data_source_info'].sample_data else 'None'}")

            return {
                "data_source_info": state['data_source_info'],
                "status": AgentStatus.PROFILING,
                "current_step": "profile_data",
                "messages": [{"role": "system", "content": f"Loaded {len(data)} rows (full_scan={full_scan})"}]
            }
...
IMPORTANT: Return ONLY the JSON object above. No markdown, no code blocks, no explanations."""
    ...
    async def _execute_validations(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            # Combine engine logs and summary
            all_logs = engine.execution_logs + [{"role": "system", "content": f"Validation complete. Passed: {passed}, Failed: {failed}"}]
