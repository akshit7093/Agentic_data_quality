...
# Max LLM output tokens per call (your model supports ~124 088 context tokens;
# this caps the *generation* length to prevent hallucination spirals)
LLM_MAX_TOKENS = 2048
...
# LangGraph recursion limit (each "step" = 2 graph nodes, so 50 ≈ 25 iterations)
...
4. You MUST output EXACTLY ONE query JSON block per response. Wait for the system to return the execution results before outputting your next query. Do NOT generate multiple queries at once.
...
5. You MUST output EXACTLY ONE query JSON block per response. Wait for the query results before generating the next one. Do NOT generate multiple queries at once.
...
class DataQualityAgent:
    ...
    def _sanitize_llm_response(raw: str) -> str:
        """
        Strip hallucinated multi-turn continuations from LLM output.

        Small / local LLMs often hallucinate an entire fake conversation:
        they emit a valid first action, then append "ASSISTANT: <next action>"
        repeatedly.  When this bloated text is stored in the message state,
        it snowballs on every iteration, eventually hitting the recursion
        limit or the token limit.

        This sanitiser:
          1. Truncates at the first 'ASSISTANT:' boundary (hallucinated turn).
          2. If multiple structural tags are present, keeps only the first.
          3. If multiple JSON action blocks are present, keeps only the first.
        """
        if not raw:
            return raw

        # -- 1. Truncate at first hallucinated turn --
        hallucination_boundary = re.search(
            r'\n\s*ASSISTANT\s*:', raw, re.IGNORECASE
        )
        if hallucination_boundary:
            raw = raw[:hallucination_boundary.start()].rstrip()
            logger.debug(
                "Sanitised LLM response: stripped hallucinated continuation "
                f"at char {hallucination_boundary.start()}"
            )

        # -- 2. Keep only the first structural tag --
        for tag in ('METADATA', 'REPORT'):
            pattern = rf'(<{tag}>[\s\S]+?</{tag}>)'
            matches = list(re.finditer(pattern, raw, re.IGNORECASE))
            if len(matches) > 1:
                first_end = matches[0].end()
                raw = raw[:first_end].rstrip()
                logger.debug(
                    f"Sanitised LLM response: stripped {len(matches) - 1} "
                    f"duplicate <{tag}> blocks"
                )

        # -- 3. Keep only the first JSON action block --
        json_blocks = list(re.finditer(
            r'```json\s*[\s\S]*?\s*```', raw, re.IGNORECASE
        ))
        if len(json_blocks) > 1:
            first_end = json_blocks[0].end()
            between = raw[first_end:json_blocks[1].start()].strip()
            raw = raw[:first_end]
            if between:
                raw += '\n' + between
            raw = raw.rstrip()
            logger.debug(
                f"Sanitised LLM response: stripped {len(json_blocks) - 1} "
                "duplicate JSON action blocks"
            )
    ...
    def _route_validation(self, state: AgentState) -> str:
        ...
        has_pending_query = bool(
            re.search(r'"action"\s*:\s*"execute(?:_query)?"', last_message)
        )
    ...
    async def _setup_connection(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            filter_context = ""
            if slice_filters:
                filter_context = f" NOTE: The user has strictly filtered this validation to {slice_filters}. All row counts and samples MUST reflect this filtered slice only."

            init_msg = (
                f"System: Connected to target '{target}'. "
                f"Schema: {json.dumps(schema, default=str)}."
                f"{filter_context} You may now begin exploration."
            )
...
If you have run all checks and investigated failures, output your final <REPORT>...</REPORT>.{conclusion_nudge}"""
    ...
    async def _execute_query_tool(self, state: AgentState) -> Dict[str, Any]:
        ...
        last_message = self._sanitize_llm_response(state["messages"][-1]["content"])

        # FIX: If the LLM bundled queries alongside a COMPLETE <METADATA> or
        # <REPORT> block, skip query execution. Return the message back so the
        # routing node can detect the structural tags and transition phases.
        ...
        if has_metadata or has_report:
            tag = "METADATA" if has_metadata else "REPORT"
            logger.info(
                f"⚠️ LLM bundled queries with <{tag}> block. "
                "Skipping query execution — routing will handle the structural tag."
            )
        ...
        if not match:
            match = re.search(
                r'(\{[\s\S]*?"action"\s*:\s*"execute(?:_query)?"[\s\S]*?\})',
                last_message
            )
        ...
        try:
            ...
            if q_type.lower() == 'sql':
                ...
                if source_type in ['postgresql', 'postgres', 'mysql', 'sqlite']:
                    try:
                        ...
                        slice_filters = getattr(state['data_source_info'], 'slice_filters', None)
                        pushdown_results = await connector.execute_raw_query(query, slice_filters=slice_filters)
            ...
            # FIX: Auto-assign rule_name when the LLM omits it —
            #      previously required "rule_name" in action_data, so small models
            #      that omitted it would produce 0 ValidationResults → 0% score.
            validation_updates = {}
            if state["status"] == AgentStatus.VALIDATING:
                # Auto-generate rule_name from SQL if missing
                rule_name = action_data.get("rule_name")
                if not rule_name:
                    # Derive a readable name from the WHERE clause
                    where_match = re.search(r'WHERE\s+(.+?)(?:;|\s*$)', query or "", re.IGNORECASE)
                    if where_match:
                        rule_name = f"Check_{where_match.group(1)[:40].strip().replace(' ', '_')}"
                    else:
                        rule_name = f"agent_validation_{state.get('validation_steps', 0)}"
